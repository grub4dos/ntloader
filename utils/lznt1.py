...
def lznt1_decompress(buf, length_check=True):
    out = bytes()
    while buf:
...
def lznt1_compress(buf, chunk_size=0x1000):
    out = bytes()
    while buf:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LZNT1 compress/decompress')
    parser.add_argument("-d", "--decompress", action="store_true", help="decompress")
    parser.add_argument("SRC", help="Specify input")
    parser.add_argument("DST", help="Specify output")
