...
static struct e820_entry *e820buf = (void *) (intptr_t) 0x30000;
...
uint64_t relocate_memory (void *data, size_t len)
{
  ...
  memset (e820buf, 0, sizeof (struct e820_entry));
  ...
  do
  {
    ...
    params.es = BASE_SEG;
    params.edi = (((void *) e820buf) - ((void *) BASE_ADDRESS));
    ...
    DBG2 ("INT 15,e820 region [%llx,%llx) type %d\n",
          e820buf->start, (e820buf->start + e820buf->len), e820buf->type);
    ...
    while (len)
    {
      ...
      offset = (((intptr_t) data) & (PAGE_SIZE_2MB - 1));
      frag_len = (PAGE_SIZE_2MB - offset);
      if (frag_len > len)
        frag_len = len;

      /* Map copy window to destination */
      map_page (COPY_WINDOW, (dest & ~( PAGE_SIZE_2MB - 1)));

      /* Copy data through copy window */
      memcpy ((((void *) COPY_WINDOW) + offset), data, frag_len);

      /* Map original page to destination */
      map_page ((((intptr_t) data) - offset), (dest & ~(PAGE_SIZE_2MB - 1)));
